...
def guild_check(_custom_commands):
    async def predicate(ctx):
        return _custom_commands.get(
            ctx.command.qualified_name
        ) and ctx.guild.id in _custom_commands.get(ctx.command.qualified_name)
...
class CustomCommands(commands.Cog):
    ...
    async def add_command(self, ctx, name, *, output):
        ...
        if existing_command is None and ctx.bot.get_command(name):
            return await ctx.send(
                f"A built in command with the name {name} is already registered"
            )
        ...
        # Otherwise, we need to create the command object
            ...
            @commands.command(
                name=name, help=f"Custom command: Outputs your custom provided output"
            )
    ...
    async def remove_command(self, ctx, name):
        ...
        if (
            name not in self._custom_commands
            or ctx.guild.id not in self._custom_commands[name]
        ):
